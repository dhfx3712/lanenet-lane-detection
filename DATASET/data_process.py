...
if __name__ == "__main__":
    ...
    with open("./train.txt", 'w+') as file:
        print (f'')
        #os.listdir 遍历文件以及文件夹。多了.ds以及jpg照片文件
        for images_dir in os.listdir("./test_images"):
            dir_name = os.path.join("./test_images",  "annotations")
            print (f'dir_name : {dir_name},{os.listdir("./test_images")}')
            for annotations_dir in os.listdir(dir_name):
                json_dir = os.path.join(dir_name, annotations_dir)
                print (f'json_dir : {json_dir},{os.listdir(dir_name)}')
                if os.path.isdir(json_dir):
                    # train_rows = moveImageTodir(json_dir, "./test_images/data", str(count).zfill(4))
                    train_rows = moveImageTodir(json_dir, "./data", str(count))
